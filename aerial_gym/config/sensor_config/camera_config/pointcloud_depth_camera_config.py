...
class PointCloudDepthCameraConfig(BaseSensorConfig):
    ...
    calculate_depth = (
        False  # Get a depth image and not a range image. False will result in a range image
    )
    return_pointcloud = True  # Return a pointcloud instead of an image. Above depth option will be ignored if this is set to True
